...
export const useCheckCodeError = (res, setDeleteUser, setOkCheck, setUser, setReloadPageError, setRes) => {
  ...
  //! --------------------------------------------------------------------------
  //! ----> 200: usuario borrado includes('Error, we could't delete user') <----
  //! --------------------------------------------------------------------------

  if (res?.data?.delete?.includes("Error, we could't delete user")) {
    Swal.fire({
      icon: "error",
      title: "No correct Code ❎.",
      text: "User was not deleted. Please, try again.",
      showConfirmButton: false,
      timer: 2500,
    });
    setRes(() => {});
  ...
  //! -----------------------------------------------------------------
  //! ----> 200: usuario no borrado includes ('Delete user done') <----
  //! -----------------------------------------------------------------

  if (res?.data?.delete?.includes("Delete user done")) {
    setDeleteUser(() => true);
    Swal.fire({
      icon: "error",
      title: "No correct Code ❎.",
      text: "Your user was deleted. Please, register again.",
      showConfirmButton: false,
      timer: 2500,
    });
    setRes(() => {});
